...
class BinanceDataDumper:
    ...
    def __init__(self, logger, spotTickers=None, umFuturesTickers=None, dataTypes=["klines", "metrics", "fundingRate"], timeframes=["1m"], pathToSave=None):
        self.logger = logger
        ...
        if all([timeframe in BinanceDataDumper._DATA_FREQUENCY_ENUM for timeframe in timeframes]):
            ...
            self.logger.info(f"Timeframes set to: {self.timeframes}")
        else:
            self.logger.error("Incorrect timeframes provided")
        ...
        self.logger.info(f"Data will be saved to: {self.pathToSave}")

    def dumpData(
            self,
            dateStartPriceOHLCV=None,
            dateEndPriceOHLCV=None,
            dateStartMetrics=None,
            dateEndMetrics=None,
            dateStartFundingRate=None,
            dateEndFundingRate=None
        ):
        self.logger.info("Starting data download process")
        queue = Queue()
        queue = self._getAllLinksPaths(
            queue=queue,
            dateStartPriceOHLCV=dateStartPriceOHLCV,
            dateEndPriceOHLCV=dateEndPriceOHLCV,
            dateStartMetrics=dateStartMetrics,
            dateEndMetrics=dateEndMetrics,
            dateStartFundingRate=dateStartFundingRate,
            dateEndFundingRate=dateEndFundingRate
        )
        numThreads = 25
        self.logger.info(f"Launching {numThreads} threads for data download")
            ...
            try:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        self.logger.info(f"Downloaded file from URL: {url}")
                        ...
                        self.logger.info(f"No data available for {url}")
                except Exception as e:
                    self.logger.error(f"Error downloading {url}: {e}")
                finally:
                    queue.task_done()
            except:
                self.logger.info("Queue is empty, thread shutting down")
                break

    def _getAllLinksPaths(
            self,
            queue,
            dateStartPriceOHLCV=None,
            dateEndPriceOHLCV=None,
        ...
        self.logger.info("Gathering all links and paths for download")
        if self.spotTickers:
            if "klines" in self.dataTypes:
                for spotTicker in self.spotTickers:
                    self.logger.info(f"Processing spot ticker: {spotTicker}")
                    queue = self._getKlinesLinksPaths(ticker=spotTicker, queue=queue,
                                                      spot=True, dataType="klines",
                                                      dateStart=dateStartPriceOHLCV, dateEnd=dateEndPriceOHLCV)

        if self.umFuturesTickers:
            if "klines" in self.dataTypes:
                for umFuturesTicker in self.umFuturesTickers:
                    self.logger.info(f"Processing futures ticker: {umFuturesTicker} for klines")
            ...
            if "metrics" in self.dataTypes:
                for umFuturesTicker in self.umFuturesTickers:
                    self.logger.info(f"Processing futures ticker: {umFuturesTicker} for metrics")
                    queue = self._getKlinesLinksPaths(ticker=umFuturesTicker, queue=queue,
                                                      dataType="metrics", spot=False,
                                                      dateStart=dateStartMetrics, dateEnd=dateEndMetrics)

            if "fundingRate" in self.dataTypes:
                for umFuturesTicker in self.umFuturesTickers:
                    self.logger.info(f"Processing futures ticker: {umFuturesTicker} for fundingRate")
                    queue = self._getKlinesLinksPaths(ticker=umFuturesTicker, queue=queue,
                                                      dataType="fundingRate", spot=False,
                                                      dateStart=dateStartFundingRate, dateEnd=dateEndFundingRate)
        return queue

    def _getKlinesLinksPaths(
            self,
            ticker,
            queue,
            dataType="klines",
        ...
        self.logger.info(f"Getting links and paths for ticker {ticker}, data type {dataType}")
        if spot:
            if dataType == "klines":
    ...
    def _createMonthlyListOfDates(self, dateStart=None, dateEnd=None):
        ...
        self.logger.info(f"Creating monthly date list from {dateStart} to {dateEnd}")
    ...
    def _createDailyListOfDates(self, dateStart=None, dateEnd=None, metrics=False):
        ...
        if not metrics:
            if dateStart is None and dateEnd.day == 1:
                ...
                self.logger.info(f"Creating daily date list from start of month to {dateEnd}")
                return daysDates.to_list()
            else:
                daysDates = pd.date_range(start=pd.Timestamp(dateStart), end=dateEnd,
                                          freq="D").strftime('%Y-%m-%d')
                self.logger.info(f"Creating daily date list from {dateStart} to {dateEnd}")
                return daysDates.to_list()
        else:
            if dateStart is None:
                dateStart = pd.Timestamp(2017, 1, 9).date()
            daysDates = pd.date_range(start=dateStart, end=dateEnd, freq="D").strftime('%Y-%m-%d')
            self.logger.info(f"Creating daily date list for metrics from {dateStart} to {dateEnd}")
            return daysDates.to_list()
    ...
    def _setDefaultDate(self, dateStart=True):
        ...
        self.logger.info(f"Default date set to: {defaultDate}")
        return defaultDate

    def _addURLsPathsToQueue(self, queue, urls, paths):
        self.logger.info(f"Adding {len(urls)} URLs and paths to the queue")
